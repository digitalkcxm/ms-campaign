...
export default (database, logger, redis) => {
  ...
  router.post('/', checkSchema(create), applyRules, async (req, res, next) => {
    try {
      ...
      const { name, create_by, id_workflow, draft, repeat, start_date, repetition_rule, filter, end_date, id_phase, ignore_open_tickets, first_message, negotiation, file_url } = req.body

      const result = await campaignController.create(req.company, XTenantID, name, create_by, id_workflow, draft, repeat, start_date, repetition_rule, filter, end_date, id_phase, ignore_open_tickets, first_message, negotiation, file_url)
